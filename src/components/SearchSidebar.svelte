<script lang="ts">
  ...
  $effect(() => {
    ...
    onlyThread = params.get("only_thread") === "true";
  ...
  function getUrl(newParams: Record<string, string | null>) {
    ...
    // 保持only_thread参数
    if (onlyThread && !("only_thread" in newParams)) {
      url.searchParams.set("only_thread", "true");
    }
    ...
    <div class="p-6 flex flex-col">
      <form action="/search" method="get">
        ...
        <div class="flex flex-row gap-1.5">
          ...
          <button
            type="submit"
            class="bg-blue-700 hover:bg-blue-600 active:bg-blue-800 text-white px-4 py-1.5 rounded shrink-0 whitespace-nowrap transition-colors cursor-pointer">搜索</button>
        </div>
        {#if scope !== "moderation"}
          <div class="mt-5">
